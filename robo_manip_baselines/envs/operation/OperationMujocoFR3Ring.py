...
def get_target_se3(op, offset_pos):
    ...
    return pin.SE3(pin.rpy.rpyToMatrix(np.pi / 2, 0.0, np.pi / 2), target_pos)
...
class ReachPhase1(ReachPhaseBase):
    def set_target(self):
        self.target_se3 = get_target_se3(
            self.op,
            offset_pos=np.array([-0.35, 0.05, -0.05]),  # [m]
        )
        self.duration = 0.7  # [s]
...
class OperationMujocoFR3Ring:
    def setup_env(self, render_mode="human"):
        self.env = gym.make(
            "robo_manip_baselines/MujocoFR3RingEnv-v0", render_mode=render_mode
        )
